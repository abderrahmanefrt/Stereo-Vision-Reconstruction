...
# ══════════════════════════════════════════════════════════════════
#  CONFIGURATION — MODIFIE UNIQUEMENT CES VALEURS SI BESOIN
# ══════════════════════════════════════════════════════════════════
LEFT_IMG  = 'image_left_undist4.jpg'
RIGHT_IMG = 'image_right_undist4.jpg'
BASELINE  = 90.0   # mm

# Résolution des photos de calibration (portrait téléphone)
CALIB_W = 3024
CALIB_H = 4032

# Résolution de tes images stéréo (paysage téléphone)
IMG_W = 2268
IMG_H = 3024
...
def get_camera_matrix():
    if not os.path.exists('camera_K.npy'):
        print("❌ camera_K.npy introuvable — lance d'abord step1_calibration_rapide.py")
        sys.exit(1)
    ...
    print("✅ [Étape 1] K chargé depuis camera_K.npy")
    print(f"   K original (calibré sur {CALIB_W}×{CALIB_H}) :")
    print(f"   fx={K[0,0]:.1f}  fy={K[1,1]:.1f}  cx={K[0,2]:.1f}  cy={K[1,2]:.1f}")

    # ── Rescale K vers la résolution des images stéréo ────────
    ...
    print(f"\n   K rescalé pour {IMG_W}×{IMG_H} "
          f"(scale_x={scale_x:.4f}, scale_y={scale_y:.4f}) :")
    print(f"   fx={K[0,0]:.1f}  fy={K[1,1]:.1f}  cx={K[0,2]:.1f}  cy={K[1,2]:.1f}")

    # Vérification cx/cy proches du centre
    if abs(K[0,2] - IMG_W/2) > IMG_W * 0.15:
        print(f"   ⚠️  cx={K[0,2]:.0f} loin du centre ({IMG_W//2}) — vérifie CALIB_W")
    if abs(K[1,2] - IMG_H/2) > IMG_H * 0.15:
        print(f"   ⚠️  cy={K[1,2]:.0f} loin du centre ({IMG_H//2}) — vérifie CALIB_H")
...
def load_images():
    ...
    h, w = img_l.shape[:2]
    print(f"\n✅ [Étape 2] Images chargées : {w}×{h} px")
...
def reconstruct_3d(pts_l, pts_r, K):
    ...
    if best_n == 0:
        print("❌ Aucun point 3D valide — vérifie baseline et images")
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    print(f"  K rescalé : {CALIB_W}×{CALIB_H} → {IMG_W}×{IMG_H}")
    print(f"{'═'*60}")
